## eMagzines 英文杂志自动抓取
使用 GitHub Actions 自动抓取英文杂志，提供 PDF 与 EPUB 两种格式。抓取结果作为 GitHub Release Assets 发布，不会写入 Git 历史。
...
3. 在仓库的 **Settings → Actions → General** 中，确保 Workflow permissions 允许工作流写入仓库内容。
...
如果你想在本地机器上运行抓取任务（结果会发布到 GitHub Releases），可以使用提供的脚本：
